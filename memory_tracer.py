...
def parse_cmd_args():
    parser = argparse.ArgumentParser(
        ...
        description='Run a workload on the QEMU guest while writing optimized GMBE '
                    ...
                    '-----> '
                    'Start a qemu guest (that was specially prepared for '
                    'memory_tracer) using the disk image ~/oren_vm_disk2.qcow2 '
                    'and the internal snapshot `ready_for_memory_tracer`, send '
                    'the workload /bin/date from the host to the guest, and '
                    'run the workload on the guest, while writing trace records '
                    'to the analysis tool `simple_analysis`.\n\n'
                    '(2)\n'
                    'python3.7 ~/qemu_mem_tracer/memory_tracer.py '
                    '~/oren_vm_disk2.qcow2 '
                    'ready_for_memory_tracer ~/qemu_with_GMBEOO '
                    ...
                    'cat ~/tmp_example_fifo &\n'
                    'fifo_printer_pid=`echo $!`\n'
                    'python3.7 ~/qemu_mem_tracer/memory_tracer.py '
                    '~/oren_vm_disk2.qcow2 '
                    'ready_for_memory_tracer ~/qemu_with_GMBEOO '
                    '--trace_fifo_path ~/tmp_example_fifo '
                    '--workload_path_on_host /bin/date\n'
                    'kill -9 $fifo_printer_pid\n'
                    '-----> '
                    'Start a qemu guest (that was specially prepared for '
                    'memory_tracer) using the disk image ~/oren_vm_disk2.qcow2 '
                    'and the internal snapshot `ready_for_memory_tracer`, send '
